...
# census <- read.csv('./src/census_ids.csv')
...
{	
	...
	{
		# print(val)
		val <- home[home$id==cell_id,][3][1,]
		# old_home_vals[i - 1] <- val
		new_val <- slaplace( val, global_sensivity, eps_home) #, eps_home, lowest_home_count, highest_home_count)
		home[home$id==cell_id,][3][1,] <- new_val
...
write.csv(file='./laplace_home_dist.csv', x=home, row.names=F)


# library("Rcpp")
# Rcpp::sourceCpp("./smooth.cpp")
# returnVector  <- Smooth(home_cdf)
